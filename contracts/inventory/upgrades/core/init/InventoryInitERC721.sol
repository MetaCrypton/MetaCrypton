    ...
    InventoryInitOwnership
{
    ...
    function depositERC721(
        ...
        if (
            IERC165(address(this)).supportsInterface(
                IInventoryERC721Internal(address(0x00)).processDepositERC721.selector
            )
        ) {
    ...
    function withdrawERC721(
        ...
        if (
            IERC165(address(this)).supportsInterface(
                IInventoryERC721Internal(address(0x00)).processWithdrawERC721.selector
            )
        ) {
    ...
    function getERC721s_(uint256 startIndex, uint256 number) external view override returns (ERC721Struct[] memory) {
        return getERC721s(startIndex, number);
    }

    function isERC721Owner_(address token, uint256 tokenId) external view override returns (bool) {
        return isERC721Owner(token, tokenId);
    }

    function getERC721s(uint256 startIndex, uint256 number) public view override returns (ERC721Struct[] memory) {
        return _assetsSetListToERC721(_assetsSet._getAssets(startIndex, number));
    }

    function isERC721Owner(address token, uint256 tokenId) public view override returns (bool) {
    ...
    function _depositERC721(
        ...
        address token,
        uint256 tokenId
    ) internal verifyERC721Input(token) {
