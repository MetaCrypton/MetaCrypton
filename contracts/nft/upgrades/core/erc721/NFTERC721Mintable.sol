 ...
 */
contract NFTERC721Mintable is IERC721Events, IERC721Mintable, Governable, NFTERC721Upgrade {
    ...
    function mint(address owner) external override requestPermission returns (uint256 tokenId) {
        tokenId = _tokensSet._mint(_upgradesRegistry, _inventoryInterface, _inventorySetup, owner, _inventoryUpgrades);
     ...
     * - The caller must own `tokenId` or be an approved operator.
     */
    function safeMint(address owner, bytes calldata data)
        external
        override
        requestPermission
